        ...
        <div class="widget-corner" id="widget-{{ sticker.getId() }}-handle">
            <div class="widget-headline">
                <h3>
                    ...
                    <span class="header-left">&nbsp;</span><span class="header-middle">Group info</span><span class="header-right">&nbsp;</span>
                </h3>
            </div>
        </div>
        <div class="widget-body">
            <div class="widget-content">
                ...
                <div class="group-info-icon">
                    <img src="{{ site.habboImagingPath }}/habbo-imaging/badge/{{ group.getBadge() }}.gif"/></div>
                <img id="groupname-{{ group.id }}-report" class="report-button report-gn" alt="report" src="{{ site.staticContentPath }}/web-gallery/images/myhabbo/buttons/report_button.gif" style="display: none;"/>
                <h4>{{ group.getName() }}</h4>

                <p>Group created: <b>{{ group.getCreatedDate() }}</b></p>
                <p>{{ sticker.getMembersAmount() }}</b> members</p>
                {% if group.getRoomId() > 0 %}<p>
                    <a href="{{ site.sitePath }}/client?forwardId=2&amp;roomId={{ room.getId() }}" onclick="HabboClient.roomForward(this, '{{ room.getId() }}', 'private'); return false;" target="client" class="group-info-room">{% autoescape 'html' %}{{ room.getData().getName() }
                        }{% endautoescape %}</a></p>{% endif %}
                ...
                <img id="groupdesc-{{ sticker.id }}-report" class="report-button report-gd" alt="report" src="{{ site.staticContentPath }}/web-gallery/images/myhabbo/buttons/report_button.gif" style="display: none;"/>
