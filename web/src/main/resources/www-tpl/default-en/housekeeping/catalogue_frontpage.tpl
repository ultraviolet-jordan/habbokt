...
<form class="table-responsive col-md-4" method="post">
    ...
    <div class="form-group">
        ...
        <p>
            <select onkeypress="previewTS(this.value);" onchange="previewTS(this.value);" name="image" id="image">
                {% for image in images %}
                <option value="{{ image }}"{% if image == frontpageText1 %} selected{% endif %}>{{ image }}
                </option>{% endfor %}
            </select>
        </p>
    </div>
    <div class="form-group">
        ...
        <div id="ts-preview"><img src="{{ site.staticContentPath }}/c_images/Top_Story_Images/{{ frontpageText1 }}"/>
        </div>
    </div>
